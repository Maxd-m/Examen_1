...
class Password{
    ...
    function generate_passwords(int $count = 5, int $length = 16, array $opts = []): array {
        $passwords = [];
        for ($i = 0; $i < $count; $i++) {
            $passwords[] = $this->generate_password($length, $opts);
        }
        return $passwords;
    }


    function validate_password(string $password, array $opts = []): array {
        ...
        $requirements = [
            'minLength' => strlen($password) >= $opts['min_length'],
            'requireUppercase' => !$opts['upper'] || preg_match('/[A-Z]/', $password) === 1,
            'requireLowercase' => !$opts['lower'] || preg_match('/[a-z]/', $password) === 1,
            'requireNumbers' => !$opts['digits'] || preg_match('/[0-9]/', $password) === 1,
            'requireSymbols' => !$opts['symbols'] || preg_match('/[^a-zA-Z0-9]/', $password) === 1,
            'avoidAmbiguous' => !$opts['avoid_ambiguous'] || preg_match('/[Il1O0o]/', $password) === 0,
            'excludeChars' => empty($opts['exclude']) || preg_match('/[' . preg_quote($opts['exclude'], '/') . ']/', $password) === 0,
        ];
        ...
        foreach ($requirements as $key => $passed) {
            $required = match ($key) {
                'minLength' => true, // siempre requerido
                'requireUppercase' => $opts['upper'],
                'requireLowercase' => $opts['lower'],
                'requireNumbers' => $opts['digits'],
                'requireSymbols' => $opts['symbols'],
                'avoidAmbiguous' => $opts['avoid_ambiguous'],
                'excludeChars' => !empty($opts['exclude']),
            };
